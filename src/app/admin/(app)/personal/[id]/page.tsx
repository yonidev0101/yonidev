...
export default async function PersonalProjectDetailPage({
  ...
  const data = await getPersonalProjectWithRelations(id);
  if (!data) notFound();
  ...
  return (
    <div dir="rtl" className="space-y-6 max-w-5xl">
      <header>
        <Link href="/admin/personal" className="text-[12px] text-[#94A3B8] hover:text-[#2B7FFF]">
          ← פרויקטים אישיים
        </Link>
        <div className="flex items-baseline gap-3 mt-2 flex-wrap">
          ...
          <span className="text-[11px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-full bg-[#EFF6FF] text-[#2B7FFF]">
            {PERSONAL_PROJECT_STATUS_HE[data.project.status]}
          </span>
        </div>
      </header>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <Stat label="משימות פתוחות" value={openTasks.toString()} />
        <Stat label="משימות סה״כ" value={data.tasks.length.toString()} />
        <Stat label="יעד" value={data.project.targetDate ? fmtDateHe(data.project.targetDate) : "—"} />
      </div>
