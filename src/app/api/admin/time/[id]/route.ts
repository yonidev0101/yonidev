...
export async function PATCH(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (parsed.data.durationSeconds !== undefined) update.durationSeconds = parsed.data.durationSeconds;
    ...
    const [row] = await db.update(timeEntries).set(update).where(eq(timeEntries.id, id)).returning();
